...
public abstract class YetiShareCore extends antiDDoSForHost {
    ...
    public String[] scanInfo(final DownloadLink link, final String[] fileInfo) {
        ...
        if (this.isNewYetiShareVersion(null) || supports_availablecheck_over_info_page(link) || currentUrlIsFileInfoURL) {
             ...
             */
            String betterFilename = br.getRegex(">\\s*File Page Link\\s*</span>\\s*<pre>https?://[^/]+/[A-Za-z0-9]+/([^<]+)</pre>").getMatch(0);
            if (betterFilename == null) {
                betterFilename = br.getRegex(">\\s*(?:Information about|Informação sobre|Informacja o|Nazwa pliku) \"([^\"]+)\"\\s*<").getMatch(0);
            }
            if (betterFilename != null) {
    ...
    public void checkErrors(final Browser br, final DownloadLink link, final Account account) throws PluginException {
        ...
        final Regex waitBetweenDownloads = br.getRegex(">\\s*You must wait (\\d+) minutes? ((\\d+) (segundos|seconds))?[^<]*between downloads");
    ...
    protected boolean isPremiumAccount(final Account account, final Browser br) {
        String accountType = br.getRegex(">\\s*Account\\s*Type\\s*:?\\s*</[^>]*>\\s*<[^>]*>\\s*([^<]{3,})\\s*<").getMatch(0);
    ...
    /**
     * Tries to auto-find API keys in website HTML code and return account information from API! </br>
     * If this returns something which is not null, that account information will be used and not the one obtained via website.
     */
