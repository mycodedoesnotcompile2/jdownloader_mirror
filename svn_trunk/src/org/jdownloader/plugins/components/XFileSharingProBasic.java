...
@HostPlugin(revision = "$Revision: 53012 $", interfaceVersion = 2, names = {}, urls = {})
public abstract class XFileSharingProBasic extends antiDDoSForHost implements DownloadConnectionVerifier {
    ...
    protected static final String             PROPERTY_PLUGIN_api_domain_with_protocol                 = "apidomain";
     ...
     * Replaces domain in original url if that domain is known to be dead.
     */
    protected String getContentURL(final DownloadLink link) {
        ...
        try {
            ...
            String newurl = originalURL.replaceFirst("(?i)^https?://[^/]+", protocol + hostCorrected);
            if (StringUtils.containsIgnoreCase(newurl, "killcode=")) {
                newurl = newurl.replaceFirst("(?i)\\?killcode=[^&#]+", "?").replaceFirst("(?i)\\&killcode=[^&#]+", "");
                if (newurl.endsWith("?")) {
    ...
    protected boolean isServerUnderMaintenance(final Browser br) {
        return br.getHttpConnection().getResponseCode() == 500 || br.containsHTML(">\\s*This server is in maintenance mode") || br.containsHTML(">\\s*Technical Maintenance\\s*<");
    }
    ...
    protected boolean isOffline(final DownloadLink link, final Browser br) {
        ...
        final String prefix = ">\\s*([*-]\\s*)?";
        if (br.containsHTML("No such file")) {
            return true;
        } else if (br.containsHTML(prefix + "File Not Found\\s*<")) {
            return true;
        } else if (br.containsHTML(prefix + "File Deleted\\s*<")) {
            return true;
        } else if (br.containsHTML(prefix + "The file was (removed|deleted) by")) {
            return true;
        } else if (br.containsHTML(">[^<>]*Reason for deletion:?") || br.containsHTML(prefix + "File has been removed due to copyright issues\\s*<")) {
            // DDownload with emoji: <strong>🗑️ Reason for deletion</strong>
            return true;
        } else if (br.containsHTML(prefix + "The file expired")) {
            return true;
        } else if (br.containsHTML(prefix + "Sorry, we can't find the page you're looking for")) {
            return true;
        } else if (br.containsHTML(prefix + "File could not be found due to expiration or removal by the file owner")) {
            return true;
        } else if (br.containsHTML(prefix + "The file of the above link no longer exists")) {
            return true;
        } else if (br.containsHTML(prefix + "video you are looking for is not found")) {
            return true;
        } else if (br.containsHTML(prefix + "The file you were looking for doesn't exist")) {
            return true;
        } else if (br.containsHTML(prefix + "File is not? longer available as it")) {
            return true;
        } else {
            return false;
        }
    ...
    protected byte[] peekURLConnection(final URLConnectionAdapter urlConnection) throws PluginException {
        ...
        checkServerErrors(null, probeContext, getDownloadLink(), getCurrentAccount());
        return peekBytes;
    }
    ...
    protected boolean looksLikeDownloadableContent(final URLConnectionAdapter urlConnection) {
        try {
            peekURLConnection(urlConnection);
    ...
    public static String removeDiv(Plugin plugin, String input, String divStart) {
        if (divStart == null) {
            return input;
        }
        ...
        plugin.getLogger().info("removeDiv:" + divStart);
    ...
    public void handleCaptcha(final DownloadLink link, Browser br, final Form captchaForm) throws Exception {
        ...
        if (new Regex(getCorrectBR(br), Pattern.compile("\\$\\.post\\(\\s*\"/ddl\"", Pattern.CASE_INSENSITIVE)).patternFind()) {
        ...
        } else if (AbstractCloudflareTurnstileCaptcha.containsCloudflareTurnstileClass(captchaForm)) {
            if (handleCloudflareTurnstileCaptcha(link, br, captchaForm)) {
            }
        } else if (containsHCaptcha(captchaForm)) {
            if (handleHCaptcha(link, br, captchaForm)) {
            }
        } else if (containsRecaptchaV2Class(captchaForm)) {
            ...
            if (containsPlainTextCaptcha(captchaForm.getHtmlCode())) {
    ...
    public String getFilenameFromURL(final DownloadLink link) {
        try {
            String result = null;
            final Pattern url_name_pattern = Pattern.compile("/[a-z0-9]{12}/(.*?)(?:\\.html|\\?|$)");
            /**
            ...
            if (contentURL != null) {
                result = new Regex(new URL(contentURL).getPath(), url_name_pattern).getMatch(0);
            }
            if (result == null) {
                result = new Regex(new URL(link.getPluginPatternMatcher()).getPath(), url_name_pattern).getMatch(0);
            }
            return result;
    ...
    protected void checkErrorsLoginWebsite(final Browser br, final Account account) throws Exception {
        if (isServerUnderMaintenance(br)) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server is under maintenance", 30 * 60 * 1000l);
        }
    }
     ...
     * download.
     */
    protected final void checkServerErrors(final Browser br, final DownloadLink link, final Account account) throws PluginException {
    ...
    protected long parseSize(Size type, String string) {
        /* 2026-06-10: We asked an XFS admin and the default XFS size unit is kibi */
        switch (type) {
        case TRAFFIC: {
            final long ret = SizeFormatter.getSize(string, true, true);
            if (ret == -1) {
                return 0;
            }
            return ret;
            ...
            if (url_with_apikey == null) {
                ...
                try {
                    ...
                    if (apihost.equalsIgnoreCase(this.getHost())) {
                        ...
                        /* Remove property in case we had stored it. */
    ...
    protected InputField get2FALoginField(final Browser br, final Form form) {
        if (form.containsHTML("class\\s*=\\s*\"captcha_code\"") && containsPlainTextCaptcha(form.getHtmlCode())) {
            // for example filestore.me, <input type="text" name="code" class="captcha_code">
            return null;
        }
