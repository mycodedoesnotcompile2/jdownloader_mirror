...
public class SoundcloudCom extends PluginForHost {
    ...
    public String getDirectlink(final Plugin plugin, final DownloadLink link, final Account account, final Browser br, Map<String, Object> json) throws Exception {
        ...
        final Map<String, Object> chosenFormat = (Map<String, Object>) chosenTranscoding.get("format");
        final String mime_type = (String) chosenFormat.get("mime_type");
        final String chosenProtocol = chosenFormat.get("protocol").toString();
        if (preset.startsWith("aac") || StringUtils.contains((String) chosenFormat.get("mime_type"), "mp4a.40.2")) {
            link.setProperty(PROPERTY_chosen_codec, PreferredStreamCodec.AAC.name());
        } else if (preset.startsWith("mp3") || StringUtils.contains((String) chosenFormat.get("mime_type"), "mp4a.40.34")) {
            link.setProperty(PROPERTY_chosen_codec, PreferredStreamCodec.MP3.name());
        } else if (preset.startsWith("opus") || StringUtils.contains((String) chosenFormat.get("mime_type"), "opus")) {
            link.setProperty(PROPERTY_chosen_codec, PreferredStreamCodec.OPUS.name());
        } else {
            throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
        }
        link.setProperty(PROPERTY_chosen_preset, preset);
        link.setProperty(PROPERTY_chosen_protocol, chosenProtocol);
        ...
        plugin.getLogger().info("Chosen audio preset: " + preset + " - " + chosenProtocol);
        ...
        final String track_authorization = (String) json.get("track_authorization");
        final Browser br2 = br.cloneBrowser();
        final UrlQuery query = UrlQuery.parse(streamUrl);
        query.add("client_id", getClientId(null));
        if (track_authorization != null) {
            query.add("track_authorization", track_authorization);
        }
        streamUrl = URLHelper.parseLocation(new URL(streamUrl), "?" + query.toString()).toString();
        Request request = br2.createGetRequest(streamUrl);
        request.getHeaders().put(HTTPConstants.HEADER_REQUEST_ACCEPT, "*/*");
        request.getHeaders().put(HTTPConstants.HEADER_REQUEST_REFERER, "https://soundcloud.com");
        request.getHeaders().put(HTTPConstants.HEADER_REQUEST_ORIGIN, "https://soundcloud.com");
        br2.getPage(request);
        try {
