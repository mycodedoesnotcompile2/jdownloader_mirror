...
@HostPlugin(revision = "$Revision: 50978 $", interfaceVersion = 3, names = {}, urls = {})
...
public class VoeSx extends XFileSharingProBasic {
    ...
    protected String getDllinkVideohostJavaScript(DownloadLink link, Account account, Browser br, final String src) {
        final ScriptEngineManager manager = JavaScriptEngineFactory.getScriptEngineManager(this);
        final ScriptEngine engine = manager.getEngineByName("javascript");
        try {
            final String input = br.getRegex("MKGMa\\s*=\\s*(\"|')(.*?)\\1").getMatch(1);
            if (input == null) {
                return null;
            }
            final String replace = br.getRegex("=\\s*(\\[\\s*(?:'[^']{2,}'\\s*,?){5,8}\\])\\s*,").getMatch(0);
            if (replace == null) {
