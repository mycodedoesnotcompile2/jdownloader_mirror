...
public class MegafileCc extends PluginForHost {
    public MegafileCc(PluginWrapper wrapper) {
        super(wrapper);
    }

    @Override
    public String getAGBLink() {
        return "https://megafile.cc/terms";
    }
    ...
    public static String[] getAnnotationUrls() {
        ...
        for (final String[] domains : getPluginDomains()) {
            ret.add("https?://(?:www\\.)?" + buildHostsPatternPart(domains) + "/d/([A-Z0-9]+)/([a-z0-9\\-]+)");
        }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        if (!link.isNameSet()) {
            /* Fallback */
            link.setName(new Regex(link.getPluginPatternMatcher(), this.getSupportedLinks()).getMatch(0) + new Regex(link.getPluginPatternMatcher(), this.getSupportedLinks()).getMatch(1));
        }
