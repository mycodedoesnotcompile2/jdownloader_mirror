...
@HostPlugin(revision = "$Revision: 50418 $", interfaceVersion = 3, names = {}, urls = {})
public class RanozGg extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        final String filesize = br.getRegex("\">\\s*Size\\s*</div><div[^>]*>([^<]+)</div>").getMatch(0);
        if (filename != null) {
    ...
    private void handleDownload(final DownloadLink link) throws Exception, PluginException {
        ...
        if (!attemptStoredDownloadurlDownload(link, directlinkproperty)) {
            ...
            final String dllink = br.getRegex("\"(https?://st\\d+\\.[^/]+/[^/\"\\\\]+)").getMatch(0);
