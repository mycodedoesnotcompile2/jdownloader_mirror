...
public class LocknexCom extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(final CryptedLink param, ProgressController progress) throws Exception {
        ...
        if (captcha_or_password_form != null) {
            ...
            } else if (br.containsHTML(">\\s*Captcha-Klick war falsch")) {
                throw new DecrypterRetryException(RetryReason.CAPTCHA);
            }
        }
        final String title = br.getRegex("class=\"fa-duotone fa-folder-open\"[^>]*>\\s*</i>([^<]+)<span").getMatch(0);
        final String urls_json = br.getRegex("window\\.CNL_LINKS = (\\[[^\\]]+\\])").getMatch(0);
        if (urls_json == null) {
