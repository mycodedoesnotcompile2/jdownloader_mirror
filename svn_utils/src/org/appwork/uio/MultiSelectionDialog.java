...
/**
 * An {@link AbstractDialog} which allows for selecting multiple options from given list.
 *
 */
public class MultiSelectionDialog extends AbstractDialog<int[]> implements MultiSelectionDialogInterface {
    ...
    public MultiSelectionDialog(int flag, String title, String question, Object[] options, int[] preSelection, Icon icon, String okText, String cancelText, ListCellRenderer renderer) {
    ...
    private JList createListComponent() {
        ...
        final int[] preSelection = this.preSelection;
        if (preSelection != null) {
            ret.setSelectedIndices(preSelection);
        }
        return ret;
    }

    public int[] getSelectedIndices() {
        final JList listComponent = this.listComponent;
        if (listComponent != null) {
            return this.listComponent.getSelectedIndices();
        }
        final int[] preSelection = this.preSelection;
        if (preSelection != null) {
            return preSelection.clone();
        }
        return new int[0];
    }
