...
public class JNAWindowsTrustProvider implements TrustProviderInterface {
     ...
     * certificates to an in-memory store for chain building.
     * <p>
     ...
     * <li><b>CertAddEncodedCertificateToStore</b>: Minimum supported client Windows 2000 Professional; minimum supported server Windows
     * 2000 Server.</li>
     * </ul>
     * <p>
     * In practice, any Windows version that runs the JNA-based trust provider (typically Windows 7 or later in support) supports these
     * APIs.
     * </p>
     */
    protected interface WindowsCertChainCrypt32Api extends Library {
        /** Singleton instance loading crypt32.dll. */
        final static WindowsCertChainCrypt32Api INSTANCE = Native.load("crypt32", WindowsCertChainCrypt32Api.class, JNAOptions.UNICODE_SYSTEM_DLLS_ONLY);

        /**
         * Creates a certificate context from an encoded certificate. The context is not persisted to a store and must be freed by the
         * caller with {@link com.sun.jna.platform.win32.Crypt32#CertFreeCertificateContext}.
         *
         ...
         * @return A {@link CERT_CONTEXT} reference, or {@code null} on failure (use {@link com.sun.jna.Native#getLastError()} for error
         *         code).
         * @see <a href=
        ...
        /**
         * Adds an encoded certificate to a certificate store. Used by {@link JNAWindowsTrustProvider} to add intermediate certificates to
         * an in-memory store so that {@code CertGetCertificateChain} can build the full chain.
         *
         * @param hCertStore
     ...
     * <p>
     * When validation succeeded, {@link #getTrustedRootCertificate()} returns the root CA certificate from the Windows-built chain that was
     * used to establish trust (the last element of the first simple chain in {@code CERT_CHAIN_CONTEXT}). When validation failed, it
     * returns {@code null}.
     * </p>
     */
    public class JNAWindowsTrustResult extends TrustResult {
         ...
         * @param chain
         *            the certificate chain that was validated
         * @param exception
         *            null if trusted, otherwise the validation error
         * @param type
         *            SERVER or CLIENT
         * @param trustedRootCertificate
         *            the root CA that granted trust, or null if validation failed
         */
        protected JNAWindowsTrustResult(final JNAWindowsTrustProvider trustProvider, final X509Certificate[] chain, final Exception exception, final TrustResult.TrustType type, final X509Certificate trustedRootCertificate) {
