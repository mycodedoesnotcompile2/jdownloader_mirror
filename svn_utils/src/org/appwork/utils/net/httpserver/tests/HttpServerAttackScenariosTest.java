...
public class HttpServerAttackScenariosTest extends HttpServerTestBase {
    ...
    /**
     * Test: Too long GET parameters (URL should exceed default 4KB header limit)
     */
    private void testLongGetParameters() throws Exception {
        LogV3.info("Test: Too Long GET Parameters");
        lastServerException = null;
        final long startTime = System.currentTimeMillis();
        // Create a very long parameter (20KB raw, encoded will be even larger) - exceeds default 4KB header limit
        final String longParam = generateRandomString(20000);
        ...
        try {
            ...
            final int responseCode = context.getCode();
            final long elapsed = System.currentTimeMillis() - startTime;
            // Should return 413 Request Entity Too Large (exceeds 4KB default header limit)
    ...
    /**
     * Test: Large POST body (500KB) - should be within default 1MB limit
     */
    private void testLargePostBody() throws Exception {
        LogV3.info("Test: Large POST Body (500KB) - within default 1MB limit");
        lastServerException = null;
        final long startTime = System.currentTimeMillis();
        ...
        try {
            // Create 500KB data (within default 1MB limit)
            final String largeData = generateRandomString((int) ReadableBytes.Unit.KB.toKibiBytes(500));
            final String jsonData = "{\"params\":[" + Deser.toString(largeData) + "]}";
            ...
            try {
                ...
                final int responseCode = context.getCode();
                final long elapsed = System.currentTimeMillis() - startTime;
                // Should return 200 (within 1MB default limit)
                assertTrue(responseCode == 200, "Large POST (500KB) should return 200, was: " + responseCode);
                assertTrue(lastServerException == null, "Server-side: No exception expected for 500KB POST within limit, but got: " + lastServerException);
                // Timing validation: 500KB POST within limit
                // - Measured: 53-77ms across 5 runs
                // - Max allowed: 150ms (2x measured max)
                assertTrue(elapsed < 250, "500KB POST should complete within 150ms, took: " + elapsed + "ms");
                assertTrue(elapsed > 0, "Request duration should be positive, was: " + elapsed + "ms");
    ...
    /**
     * Test: Very large POST body (3MB) - exceeds default 1MB POST limit but within 10MB drain limit. Server should drain the data and
     * return 413 cleanly.
     */
    private void testVeryLargePostBodyWithinDrainLimit() throws Exception {
        LogV3.info("Test: Very Large POST Body (3MB) - exceeds 1MB POST limit, within 10MB drain limit");
        lastServerException = null;
        final long startTime = System.currentTimeMillis();
        // Allow POST method for this test
        final Set<RequestMethod> previousMethods = this.allowHttpMethods(RequestMethod.GET, RequestMethod.POST);
        try {
            // Create 3MB data (exceeds default 1MB POST limit, but within 10MB drain limit)
            final String veryLargeData = generateRandomString((int) ReadableBytes.Unit.MB.toKibiBytes(3));
            final String jsonData = "{\"params\":[" + Deser.toString(veryLargeData) + "]}";
            final String url = "http://localhost:" + this.serverPort + "/test/postData";
            // x-appwork header is already set in setupServer()
            this.httpClient.putRequestHeader(HTTPConstants.HEADER_REQUEST_CONTENT_TYPE, "application/json");
            this.httpClient.setReadTimeout((int) TimeUnit.SECONDS.toMillis(60));
            try {
                final RequestContext context = this.httpClient.post(url, jsonData);
                final int responseCode = context.getCode();
                final long elapsed = System.currentTimeMillis() - startTime;
                // Must return 413 Request Entity Too Large (exceeds 1MB POST limit)
    ...
    /**
     * Test: Very large POST body (15MB) - exceeds default 1MB POST limit AND 10MB drain limit. Server should NOT drain all data (only up to
     * drain limit) and connection should be closed.
     */
    private void testVeryLargePostBodyExceedsDrainLimit() throws Exception {
        LogV3.info("Test: Very Large POST Body (15MB) - exceeds 1MB POST limit AND 10MB drain limit");
        lastServerException = null;
        final long startTime = System.currentTimeMillis();
        // Allow POST method for this test
        final Set<RequestMethod> previousMethods = this.allowHttpMethods(RequestMethod.GET, RequestMethod.POST);
        try {
            // Create 15MB data (exceeds both 1MB POST limit AND 10MB drain limit)
            final String veryLargeData = generateRandomString((int) ReadableBytes.Unit.MB.toKibiBytes(15));
            final String jsonData = "{\"params\":[" + Deser.toString(veryLargeData) + "]}";
            final String url = "http://localhost:" + this.serverPort + "/test/postData";
            // x-appwork header is already set in setupServer()
            this.httpClient.putRequestHeader(HTTPConstants.HEADER_REQUEST_CONTENT_TYPE, "application/json");
    ...
    /**
     * Test: Many GET parameters - should be within default 4KB header limit (may exceed and return 413)
     */
    private void testManyParameters() throws Exception {
        ...
        try {
            ...
            final int responseCode = context.getCode();
            final long elapsed = System.currentTimeMillis() - startTime;
            // Should return 200 (within 4KB default header limit) or 413 if limit exceeded
    ...
    /**
     * Test: Extremely long URL - should exceed default 4KB header limit
     */
    private void testExtremelyLongUrl() throws Exception {
        LogV3.info("Test: Extremely Long URL - exceeds default 4KB header limit");
        lastServerException = null;
        final long startTime = System.currentTimeMillis();
        final StringBuilder longPath = new StringBuilder("http://localhost:" + this.serverPort + "/test/echo?message=");
        // Create a very long URL (20KB raw, encoded will be even larger) - exceeds default 4KB header limit
